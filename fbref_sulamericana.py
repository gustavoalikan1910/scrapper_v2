...
with DAG(
    dag_id="fbref_sulamericana",
    default_args=default_args,
    description="Executa o script CAPTURA_DADOS_FREF_TO_LINUX.py, sulamericana parametrizado, transforma em Delta e cria silver",
    schedule_interval="0 0 * * *",  # Cron para 9AM UTC
    start_date=datetime(2024, 12, 22),
    catchup=False,
    tags=["fbref", "sulamericana"],
) as dag:

    # Tarefa 1: Executa o script FBREF_sulamericana.py
    get_data_fbref = PythonOperator(
        task_id="Captura_Dados_Fbref_sulamericana",
        python_callable=run_fbref_script,
    )

    # Tarefa 2: Transforma os dados em Delta para a competição sulamericana
    transform_to_delta = PythonOperator(
        task_id="Salva_Delta_Table_Bronze_sulamericana",
        python_callable=run_delta_script,
    )

    # Tarefa 3: Le delta table e insere na silver
    load_silver_tables = PythonOperator(
        task_id="Carrega_silver_tables_sulamericana",
